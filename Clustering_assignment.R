...
spectral_k_wrapper <- function(x, k, d_threshold = 1) {
  ...
  U <- ee$vectors[, order(ee$values)[1:k], drop = FALSE]
...
print(gap1, method = "firstSEmax")   # prints the table and the chosen K
khat1 <- cluster::maxSE(gap1$Tab[,"gap"], gap1$Tab[,"SE.sim"])
khat1                                 # should print an integer such as 4
plot(gap1, main = "Gap statistic (hypercube demo)")  # optional plot
...
khat2 <- cluster::maxSE(gap2$Tab[,"gap"], gap2$Tab[,"SE.sim"])
khat2
plot(gap2, main = "Gap statistic (spectral on shells)")  # optional
